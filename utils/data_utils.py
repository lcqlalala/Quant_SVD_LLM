...
def _tokenize_long_corpus(tokenizer, texts, sep="\n\n"):
    """
    Tokenize long text corpora incrementally to avoid creating one ultra-long
    sequence in a single tokenizer call (which may exceed model_max_length
    warnings for long-context tokenizers).
    ...
    sep_ids = tokenizer(sep, return_tensors='pt', add_special_tokens=False).input_ids[0]
    parts = []
    bos_id = getattr(tokenizer, "bos_token_id", None)
    if bos_id is not None:
        parts.append(torch.tensor([int(bos_id)], dtype=torch.long))
    ...
    if non_empty == 0:
        return torch.empty((1, 0), dtype=torch.long)
    if sep_ids.numel() > 0 and len(parts) > 0:
        parts = parts[:-1]
...
def get_wikitext2(nsamples, seed, seqlen, tokenizer, dataset_cache_dir=None):
    ...
    trainenc = type("TokenizerWrapper", (), {"input_ids": _tokenize_long_corpus(tokenizer, traindata['text'])})()
    testenc = type("TokenizerWrapper", (), {"input_ids": _tokenize_long_corpus(tokenizer, testdata['text'])})()
...
def get_ptb(nsamples, seed, seqlen, tokenizer, dataset_cache_dir=None):
    ...
    trainenc = type("TokenizerWrapper", (), {"input_ids": _tokenize_long_corpus(tokenizer, traindata['sentence'])})()
    testenc = type("TokenizerWrapper", (), {"input_ids": _tokenize_long_corpus(tokenizer, valdata['sentence'])})()
...
def get_ptb_new(nsamples, seed, seqlen, tokenizer, dataset_cache_dir=None):
    ...
    trainenc = type("TokenizerWrapper", (), {"input_ids": _tokenize_long_corpus(tokenizer, traindata['sentence'], sep=" ")})()
    testenc = type("TokenizerWrapper", (), {"input_ids": _tokenize_long_corpus(tokenizer, testdata['sentence'], sep=" ")})()
...
def get_c4_new(nsamples, seed, seqlen, tokenizer):
    ...
    valenc_ids = _tokenize_long_corpus(tokenizer, valdata[:1100]['text'], sep=" ")
    valenc = valenc_ids[:, :(256 * seqlen)]
...
def get_test_data(name, tokenizer, seq_len=2048, batch_size = 4):
    ...
    def process_data(samples, tokenizer, seq_len, field_name):
        test_ids = _tokenize_long_corpus(tokenizer, samples[field_name])[0]
        test_ids_batch = []
